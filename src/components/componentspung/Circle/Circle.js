...
const timeRangesToAngles = (timeRanges) => {
  if (timeRanges == []) {
    return [];
  } else if (timeRanges.indexOf("0:0-24:0") !== -1) {
    console.log("true");
    return [
...
const MultipleSectors = ({ timeRanges, color, link }) => {
  ...
  const calculateGradientColors = (angle) => {
    ...
    return [
      `#${Math.round(
        bottomColorRed - (bottomColorRed - topColorRed) * y1
      ).toString(16)}${Math.round(
        bottomColorGreen - (bottomColorGreen - topColorGreen) * y1
      ).toString(16)}${Math.round(
        bottomColorBlue - (bottomColorBlue - topColorBlue) * y1
      ).toString(16)}`,
      `#${Math.round(
        bottomColorRed - (bottomColorRed - topColorRed) * y2
      ).toString(16)}${Math.round(
        bottomColorGreen - (bottomColorGreen - topColorGreen) * y2
      ).toString(16)}${Math.round(
        bottomColorBlue - (bottomColorBlue - topColorBlue) * y2
      ).toString(16)}`,
      `#${Math.round(
        bottomColorRed -
          (bottomColorRed - topColorRed) *
            (y1 + 0.05 * (1 - Math.cos((angle.startAngle * Math.PI) / 180)))
      ).toString(16)}${Math.round(
        bottomColorGreen -
          (bottomColorGreen - topColorGreen) *
            (y1 + 0.05 * (1 - Math.cos((angle.startAngle * Math.PI) / 180)))
      ).toString(16)}${Math.round(
        bottomColorBlue -
          (bottomColorBlue - topColorBlue) *
            (y1 + 0.05 * (1 - Math.cos((angle.startAngle * Math.PI) / 180)))
      ).toString(16)}`,
      `#${Math.round(
        bottomColorRed -
      ...
      `#${Math.round(
        bottomColorRed -
          (bottomColorRed - topColorRed) *
            (y2 + 0.05 * (1 - Math.cos((angle.endAngle * Math.PI) / 180)))
      ).toString(16)}${Math.round(
        bottomColorGreen -
          (bottomColorGreen - topColorGreen) *
            (y1 + 0.05 * (1 - Math.cos((angle.endAngle * Math.PI) / 180)))
      ).toString(16)}${Math.round(
        bottomColorBlue -
          (bottomColorBlue - topColorBlue) *
            (y1 + 0.05 * (1 - Math.cos((angle.endAngle * Math.PI) / 180)))
    ...
    <View style={styles.container}>
      <Svg height={300} width={300}>
        ...
        {angles.map((angle, index) => {
          const [topColor, bottomColor, a, b, c, d] =
            calculateGradientColors(angle);
        ...
        {angles.map((angle, index) => {
          const radius = 150;
          const x1 =
            radius * Math.cos(((angle.startAngle - 90) * Math.PI) / 180);
          const y1 =
            radius * Math.sin(((angle.startAngle - 90) * Math.PI) / 180);
          ...
          const pathData = `
            ...
            A ${radius},${radius} 0 ${largeArcFlag} 1 ${radius + x2},${
            radius + y2
          }
            Z
          `;
        ...
        {angles.map((angle, index) => {
          ...
          const x1 =
            (radius - miniradius) *
            Math.cos(((angle.startAngle - 90) * Math.PI) / 180);
          const y1 =
            (radius - miniradius) *
            Math.sin(((angle.startAngle - 90) * Math.PI) / 180);
          return (
            <Circle
              key={index}
              cx={radius + x1}
        ...
        {angles.map((angle, index) => {
          ...
          const x1 =
            (radius - miniradius) *
            Math.cos(((angle.endAngle - 90) * Math.PI) / 180);
          const y1 =
            (radius - miniradius) *
            Math.sin(((angle.endAngle - 90) * Math.PI) / 180);
          return (
            <Circle
              key={index}
              cx={radius + x1}
        ...
        <Image
          source={{
            uri:
              "https://firebasestorage.googleapis.com/v0/b/wherenext-24624.appspot.com/o/images%2F" +
              link.slice(81),
          }}
          style={{
            width: imageRadius * 2,
